...
def solution_B5():
    ...
    # YOUR CODE HERE
    lr_schedule = tf.keras.callbacks.LearningRateScheduler(schedule=lambda epoch: 1e-3 * 10**(epoch/20))
    optimizers = tf.keras.optimizers.SGD(lr=1e-3, momentum=0.9)
    model.compile(loss = tf.keras.losses.Huber(),optimizer=optimizers,metrics=['mae'])
    model.fit(train_set,batch_size = 32, epochs = 50,callbacks=[lr_schedule])
    return model
